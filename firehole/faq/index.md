...
description: Common questions about Firehole setup, customization, and use.
---
...
## How do I dial the colour back?

Firehole is already quiet in type, layout, and motion — the energy lives in the palette, and that's settings, not code. Run more sections on the bright **Cream** and **White** neutral schemes, and reserve the saturated **Orange** and **Teal** blocks for the moments that earn them. The heading scale already sits at 100%, so leave the type alone. To go further still, mute the Orange/Teal schemes' saturation under **Theme settings → Colors**.
...
**Theme settings → Colors.** Each scheme edits independently across ten roles — background, text, accent text, two button pairs, and inputs. The `accent_text` role is contrast-tuned per scheme; if you change a scheme's background, re-check its accent against it. See [Customizing → Color schemes](../customizing/#color-schemes).

## What fonts does Firehole use?

The Firehole preset uses Oswald (headings), Source Serif 4 (body), Work Sans (subheadings, labels, buttons). All three axes swap independently at **Theme settings → Typography** from Shopify's font library. The scale, capitalization, and tracking settings — not the font files — carry the register.
...
## Does Firehole support multiple languages and currencies?
...
## Is Firehole compatible with Shopify apps?

Yes. Firehole supports Online Store 2.0 app blocks — apps that ship theme blocks can be added to the product page's info column and to sections that accept app blocks. Apps requiring manual code edits should follow the app's own documentation.
