...
description: How to install Firehole from the Shopify Theme Store, load demo content, and publish your first version.
---
...
1. Open the Shopify Theme Store and find Firehole.
2. Click **Try theme**. Firehole installs to your store as an unpublished theme.
3. From the Shopify admin, go to **Online Store → Themes**.
4. Find Firehole in the **Theme library** section.
5. Click **Customize** to open the theme editor.

You can run Firehole side-by-side with your current published theme. Nothing on your storefront changes until you click **Publish**.

## Load demo content

Firehole installs with the Firehole preset configured with sample copy, placeholder images, and section settings tuned to its register. New stores see the preset on first install. The demo store is branded **Headwaters**, a premium outdoor outfitter; see [Presets](../presets/) for what the preset configures.
...
Firehole expects products with:
...
Firehole's filter UI (sidebar, top bar, or drawer — set on the collection page section) renders whatever filters your store publishes. Out of the box that's **Availability** and **Price**. To add option-based filters — size, color, product type, vendor:
...
- **Homepage slideshow or hero** — Replace placeholder images and copy. The heading display-size setting is where the big landscape moments live.
...
3. Click **Actions → Publish** on the Firehole theme.
4. Confirm. Your storefront switches to Firehole immediately.
