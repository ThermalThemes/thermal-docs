...
description: Common questions about Prismatic setup, customization, and use.
---
...
The loudness is settings, not code. Three moves: drop **Heading size scale** (Theme settings → Typography) down toward 100%, set **Heading capitalization** to `none`, and stop alternating color schemes between sections (run more sections on a single light scheme). Square corners and mono labels can stay — they read as precision at any volume.
...
## Does Prismatic support multiple languages and currencies?
...
## Is Prismatic compatible with Shopify apps?

Yes. Prismatic supports Online Store 2.0 app blocks — apps that ship theme blocks can be added to the product page's info column and to sections that accept app blocks. Apps requiring manual code edits should follow the app's own documentation.
