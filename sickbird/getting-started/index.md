...
description: How to install Sickbird from the Shopify Theme Store, load demo content, and publish your first version.
---
...
1. Open the Shopify Theme Store and find Sickbird.
2. Click **Try theme**. Sickbird installs to your store as an unpublished theme.
3. From the Shopify admin, go to **Online Store → Themes**.
4. Find Sickbird in the **Theme library** section.
5. Click **Customize** to open the theme editor.

You can run Sickbird side-by-side with your current published theme. Nothing on your storefront changes until you click **Publish**.

## Load demo content

Sickbird installs with a single preset configured with sample copy, placeholder images, and section settings tuned to the theme's register — uppercase Oswald display type, hard scheme inversions, monospace labels. New stores see the preset on first install.
...
Sickbird expects products with:
...
Sickbird's filter UI (sidebar, top bar, or drawer — set on the collection page section) renders whatever filters your store publishes. Out of the box that's **Availability** and **Price**. To add option-based filters — size, color, product type, vendor:
...
3. Click **Actions → Publish** on the Sickbird theme.
4. Confirm. Your storefront switches to Sickbird immediately.
