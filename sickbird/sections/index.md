...
description: Every section in Sickbird's library, grouped by category, with one-line descriptions and links to per-section settings documentation.
---

# Sections reference

Sickbird ships 27 drop-in sections plus the header, footer, and announcement bar, built on the thermal-base substrate. Sections are grouped into layout, marketing, product, and content categories. Every section is documented with its settings, blocks, and intended use.
...
Every section ships with a `color_scheme` picker. Alternating schemes hard from section to section — paper, then ink, then paper, then reef green — is the primary register lever. Use the orange scheme at most once per page; it works like a trail marker, not a wash.
